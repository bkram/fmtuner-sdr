...
bool RTLSDRDevice::connect() {
  ...
  return true;
#else
  std::cerr << "[SDR] direct RTL-SDR source not available in this build "
               "(enable FM_TUNER_ENABLE_RTLSDR)\n";
  return false;
#endif
}
...
bool RTLSDRDevice::setFrequency(uint32_t freqHz) {
  ...
  return rtlsdr_set_center_freq(
             reinterpret_cast<rtlsdr_dev_t *>(m_deviceHandle), freqHz) == 0;
#else
  (void)freqHz;
  return false;
#endif
...
bool RTLSDRDevice::setSampleRate(uint32_t rate) {
  ...
  return rtlsdr_set_sample_rate(
             reinterpret_cast<rtlsdr_dev_t *>(m_deviceHandle), rate) == 0;
#else
  (void)rate;
  return false;
#endif
...
bool RTLSDRDevice::setFrequencyCorrection(int ppm) {
  ...
  return rtlsdr_set_freq_correction(
             reinterpret_cast<rtlsdr_dev_t *>(m_deviceHandle), ppm) == 0;
#else
  (void)ppm;
  return false;
#endif
...
bool RTLSDRDevice::setGainMode(bool manual) {
  ...
  return rtlsdr_set_tuner_gain_mode(
             reinterpret_cast<rtlsdr_dev_t *>(m_deviceHandle),
             manual ? 1 : 0) == 0;
#else
  (void)manual;
  return false;
...
bool RTLSDRDevice::setGain(uint32_t gainTenthsDb) {
  ...
  if (!m_supportedGains.empty()) {
    auto it = std::lower_bound(m_supportedGains.begin(), m_supportedGains.end(),
                               gain);
  ...
  return rtlsdr_set_tuner_gain(reinterpret_cast<rtlsdr_dev_t *>(m_deviceHandle),
                               gain) == 0;
#else
  (void)gainTenthsDb;
  return false;
...
bool RTLSDRDevice::setAGC(bool enable) {
  ...
  return rtlsdr_set_agc_mode(reinterpret_cast<rtlsdr_dev_t *>(m_deviceHandle),
                             enable ? 1 : 0) == 0;
#else
  (void)enable;
  return false;
