...
public:
  ...
  bool start();
  void stop();
  void updateSignal(float level, bool stereo, bool forcedMono, int cci = -1,
                    int aci = -1);
  void updatePilot(int pilotTenthsKHz);
  void updateRDS(uint16_t blockA, uint16_t blockB, uint16_t blockC,
...
private:
  ...
  std::string processCommand(const std::string &cmd, bool authenticated,
                             bool guestSession);
